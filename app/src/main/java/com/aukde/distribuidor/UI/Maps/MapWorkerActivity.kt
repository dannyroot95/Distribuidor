...
class MapWorkerActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private var mWorkerProvider : WorkerProvider = WorkerProvider()
    private lateinit var mCurrentLatLng: LatLng
    private var mCameraListener: GoogleMap.OnCameraIdleListener? = null
    private val mWorkersMarkers: MutableList<Marker> = ArrayList()
    private val mWorkersLocation: ArrayList<UsersLocation> = ArrayList<UsersLocation>()
        ...
        mGeofireProvider.getActiveDrivers(mCurrentLatLng, 10.0).addGeoQueryEventListener(object :
            ...
            override fun onKeyEntered(key: String?, location: GeoLocation?) {
                ...
                val marker = mMap.addMarker(
                    MarkerOptions().position(workerLatLng).title("Trabajando")
                        .icon(BitmapDescriptorFactory.fromResource(R.drawable.worker_icon))
                )
                ...
                mWorkersLocation.add(usersLocation)
                ...
                for (marker in mWorkersMarkers) {
                    if (marker.tag != null) {
                        if (marker.tag == key) {
                            ...
                            mWorkersMarkers.removeAt(getPositionWorker(key!!))
                            return
                        }
                    }
                }
    ...
    private fun getWorkersInfo() {
        for (marker in mWorkersMarkers) {
            mWorkerProvider.getWorker(marker.tag.toString())
                .addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        ...
                        if (mCounter == mWorkersMarkers.size) {
                            mMap.setInfoWindowAdapter(
                                PopupAdapter(
                                    this@MapWorkerActivity,
    ...
    private fun getPositionWorker(id: String): Int {
        var position = 0
        for (i in mWorkersLocation.indices) {
            if (id == mWorkersLocation[i].id) {
                position = i
                break
            }
        }
